...
- (void)_setTopView
{
    _scrollView.contentSize=CGSizeMake(KWidth*4,0);
    for (int i=1; i<=4; i++) {
        UIImageView *imageView=[[UIImageView alloc]initWithFrame:CGRectMake((i-1)*KWidth, 0, KWidth, 125)];
    ...
    _pc=[[UIPageControl alloc]initWithFrame:CGRectMake(60, 80,200,60)];
    _pc.numberOfPages=4;
...
-(void)pageAction
{
    ...
    [_scrollView setContentOffset:CGPointMake(xNum*KWidth, 0)];
...
-(void)timeAction
{
    ...
    [_scrollView setContentOffset:CGPointMake(_page*KWidth, 0)];
